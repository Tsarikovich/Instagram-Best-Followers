...
import pyotp
from credentials import login, password, base32secret
import json


def get_following_likes_list(followers_likes_list):
    result = followers_likes_list

    my_following_names = [usershort.username for usershort in cl.user_following(str(cl.user_id)).values()]
...
def get_followers_likes_list():
    ...
    my_followers_likes = {usershort.username: 0 for usershort in cl.user_followers(str(cl.user_id)).values()}
    ...
    my_followers_likes = dict(sorted(my_followers_likes.items(), key=lambda item: item[1]))
